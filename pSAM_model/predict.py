...
taskDir = sys.argv[1]
fastafile = readFa("./task/"+taskDir+"/inputFile.fasta")
max_len = 2000
for line in fastafile:
	data = list(line)
	if len(data[1]) > max_len:
		out = open("./task/"+taskDir+"/merged.data.txt", "w")
...
for line in fastafile:
	...
	out = open( "./task/"+taskDir+"/"+tmpid+".res.CPT.scores", "w")
	...
	pro2pos2trunc = {}
	pro2pos2delta = {}
	with open( "./task/"+taskDir+"/"+tmpid+".res.CPT.scores") as f:
		#f.readline()
		for line in f:
			data = line.strip('\n').split('\t')
	...
	out = open( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores", "w")
	with open( "./task/"+taskDir+"/"+tmpid+".res.CPT.scores") as f:
	...
	copyfile( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores",  "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.tmp")
	###########write a circle to find all regions
	allvalues, alllines = [], []
	finalscore = 0
	with open( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores") as f:
		for line in f:
			data = line.strip('\n').split('\t')
	...
	while sorted(allvalues, reverse = True)[0] > 0.1:
		...
		out = open( "./task/"+taskDir+"/"+tmpid+".res.CPT.scores.tmp", "w")
		for i in range(len(all_line)):
			out.write(all_line[i]+'\t'+str(score[i][1])+'\n')
		...
		pro2pos2trunc = {}
		pro2pos2delta = {}
		with open( "./task/"+taskDir+"/"+tmpid+".res.CPT.scores.tmp") as f:
			#f.readline()
			for line in f:
				data = line.strip('\n').split('\t')
				if data[0] not in pro2pos2score:
		...
		out = open( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.regions", "w")
		allvalues = []
		with open( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.tmp") as f:
			for line in f:
				line = line.strip('\n')
				data = line.split('\t')
				if data[0] in pro2pos2trunc:
					if int(data[1]) in pro2pos2trunc[data[0]]:
						...
						out.write(line+'\n')
		out.close()
		copyfile( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.regions",  "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.tmp")
...
for line in fastafile:
	...
	tmpid = data[0].split(' ')[0]
	seque = data[1]
	with open("./task/"+taskDir+"/"+tmpid+'.res.CPT.fasta','w',newline='\n') as f:
		f.write('>'+tmpid+'\n'+seque+'\n')
	infile = "./task/"+taskDir+"/"+tmpid+'.res.CPT.fasta'
	outdir = "./task/"+taskDir+"/"
...
for line in fastafile:
	...
	if len(data[1]) < max_len:
		...
		with open("./task/"+taskDir+"/"+tmpid+".res.CPT.otherPred.scores") as f:
			otherPredScores = f.readline().strip('\n')
			
		if os.path.exists("./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.regions"):
			with open( "./task/"+taskDir+"/"+tmpid+".res.CPT.delta.scores.regions") as f:
				for line in f:
					data = line.strip('\n').split('\t')
...
with open("./task/"+taskDir+"/runInfo.txt","a") as f:
	f.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))

with open("./task/"+taskDir+"/runInfo.txt") as f:
