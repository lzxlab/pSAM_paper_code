...
test_data, test_label = read_data(sys.argv[3])
...
roc_file(t_score, t_label, num=sys.argv[1].split(".")[-3]+".train")

score = model1.predict(val_ohe)
t_label = [int(i) for i in vali_label]
t_score = [i[1] for i in score]
print(roc_auc_score(t_label,t_score))
roc_file(t_score, t_label, num=sys.argv[1].split(".")[-3]+".valid")

score = model1.predict(test_ohe)
t_label = [int(i) for i in test_label]
t_score = [i[1] for i in score]
print(roc_auc_score(t_label,t_score))
roc_file(t_score, t_label, num=sys.argv[1].split(".")[-3]+".test")

model1.save_weights('../2.TrainValidTest/model.'+sys.argv[1].split(".")[-3]+'.h5')
json = open('../2.TrainValidTest/model.'+sys.argv[1].split(".")[-3]+'json',"w")
json.write(model1.to_json())
json.close()

loss = history1.history["loss"]
np.savetxt('../2.TrainValidTest/model.loss.'+sys.argv[1].split(".")[-3],loss)
valloss = history1.history["val_loss"]
np.savetxt('../2.TrainValidTest/model.valloss.'+sys.argv[1].split(".")[-3],valloss)
