...
if sys.argv[4] == "CNNBiLSTMAtten1":
		...
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					out1 = open("../1.cvroc/"+sys.argv[1].split("subloc.")[1].split(".txt")[0]+"_"+sys.argv[4]+"_"+str(co)+"_"+str(dr)+"_"+str(do)+"_"+str(i)+".roc", "w")
					...
					out1.write(sys.argv[1].split("subloc.")[1].split(".txt")[0]+"\t"+sys.argv[4]+"\t"+str(co)+"\t"+str(dr)+"\t"+str(do)+"\t"+str(i)+'\t'+str(roc1)+'\t'+str(roc2)+'\t'+str(roc3)+'\n')
					out1.close()
	...
	repeatime = int(sys.argv[5])
	cnnnodes = [32, 64, 128, 256]
	drate = [0.3, 0.4, 0.5, 0.6, 0.7]                      #[0.3, 0.4, 0.5, 0.6, 0.7]
	lstmnodes = [64, 128, 256, 512]                        #[64, 128, 256, 512]
	for co in cnnnodes:
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
