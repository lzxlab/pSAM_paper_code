...
roc_file(t_score, t_label, num=sys.argv[1].split(".")[-1]+".alldata")

model1.save_weights('../2.TrainValidTest/model'+sys.argv[1].split(".")[-1]+'.alldata.h5')
json = open('../2.TrainValidTest/model.'+sys.argv[1].split(".")[-1]+'.alldata.json',"w")
json.write(model1.to_json())
json.close()

loss = history1.history["loss"]
np.savetxt('../2.TrainValidTest/model.'+sys.argv[1].split(".")[-1]+'.alldata.loss.txt',loss)
